...
def main():
    ...
    if not data:
        print("❌ 經過多次嘗試後仍無法連線證交所 API")
        requests.post(DISCORD_WEBHOOK_URL, json={"content": f"📅 {today} 報告：證交所伺服器目前繁忙或維護中，建議於開盤時段再試。"})
        return

    all_stocks = pd.DataFrame(data)
    pool = all_stocks[['證券代號', '證券名稱']].values.tolist()
    print(f"成功連線！總計掃描 {len(pool)} 檔標的...")
